...
- El node_1_1 sols te 1 core
- El node_1_2 i node_1_4 tenen 4 cores
- El node_1_3 te dos cores
...
> El `rank` pot-ser siga el més important, ja que ens servirà per a identificad cadascun del processos funcionant.
---
...
- Amb l'objecte `MPI.COMM_WORLD` vist anteriorment. Aquest objecte podem entendre'l com la part física de la xarxa que comunica els porcessos en execució
...
Calculem els tamanys. `root` crea el vector i es fa un _broadcast_ del vector per a que tots el tinguen:
...
El procés root te dades i eixes dades son rebudes per tots
...
En aquest cas partim de que cada node (per exemple 4 nodes) te un escalar (un valor simple). Al reunir-lo el root tindrà un vector de 4 elements 
